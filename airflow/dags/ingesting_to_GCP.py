...
URL_TEMPLATE = (
    URL_PREFIX + "/yellow_tripdata_{{ macros.ds_format(macros.ds_add(ds, -90), '%Y-%m-%d', '%Y-%m') }}.parquet"
)
...
OUTPUT_FILE_TEMPLATE = (
    AIRFLOW_HOME + "/output_{{ macros.ds_format(macros.ds_add(ds, -90), '%Y-%m-%d', '%Y-%m') }}.parquet")

OUTPUT_FILE_NAME = "output_{{ macros.ds_format(macros.ds_add(ds, -90), '%Y-%m-%d', '%Y-%m') }}.parquet"
